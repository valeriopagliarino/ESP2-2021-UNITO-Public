...
void esper6()
{
    ...
    for (int j = 0; j < 5; ++j)
    {
        ...
        for (int i = j*s ; i <= (s*(j+1)-1); i++)
        {
            ...
            t[j]->SetMarkerStyle(20+j);
            printout << i << "\t" << Vce[i] << "\t \t" << Ic[i] << std::endl;
            k++;
        }
        printout << std::endl << "-----------------------------" << std::endl;
        t[j]->Sort();
        if (j==3) t[j]->GetXaxis()->SetLimits(0.,11);
        if (j==3) t[j]->Draw();
    }
    ...
    */
    ...
    cIn->SetGrid();
    cIn->Draw();
    cIn->Write("cIn");*/ //parte commentata: ciclo for che disegna le 18 caratteristiche in ingresso
    ...
    TGraphErrors * fb = new TGraphErrors(5);
        fb->SetName("fb");
        float Vce1 = Vce[0+s*2];
        fb->SetTitle("Vce ~ "  " V ");

    for (int i = 0; i < 5; ++i)
        {
            fb->SetPoint(i, Vbe[0+i*s], Ib[0+i*s]);
            fb->SetPointError(i, Vbe_err[0+i*s], Ib_err[0+i*s]);
            fb->SetMarkerStyle(20);
        }
    fb->Sort();
    fb->Write("fb") ;
    ...
    fb->SetLineColor(4);
    fb->SetLineWidth(1);
    fb->GetXaxis()->SetTitle("Vbe [V]");
    fb->GetYaxis()->SetTitle("Ib [A]");
    fb->SetTitle("Caratteristica in ingresso Ib = f(Vbe)");
    fb->GetXaxis()->SetLimits(0.4 , 1.8);
    fb->GetYaxis()->SetLimits(3e-5, 9e-4);
    fb->Draw();

    TGraphErrors * fb1 = new TGraphErrors(5);
    fb1->SetName("fb1");
    fb1->SetTitle("Vce ~ "  " V ");

    for (int i = 0; i < 5; ++i)
        {
            fb1->SetPoint(i, Vbe[8+i*s], Ib[8+i*s]);
            fb1->SetPointError(i, Vbe_err[8+i*s], Ib_err[8+i*s]);
            fb1->SetMarkerStyle(21);
        }
    fb1->Sort();
    fb1->Write("fb1") ;
    ...
    TGraphErrors * fb2 = new TGraphErrors(5);
    fb2->SetName("fb2");
    fb2->SetTitle("Vce ~ "  " V ");

    for (int i = 0; i < 5; ++i)
        {
            fb2->SetPoint(i, Vbe[17+i*s], Ib[17+i*s]);
            fb2->SetPointError(i, Vbe_err[17+i*s], Ib_err[17+i*s]);
            fb2->SetMarkerStyle(22);
        }
    fb2->Sort();
    fb2->Write("fb1") ;
    ...
    auto * l1 = new TLegend(0.1,0.7,0.48,0.9);
    l1->AddEntry("fb", "Vce ~  0,005 V ", "r");
    l1->SetHeader("Legenda");
    l1->AddEntry("fb1", "Vce ~  4,000 V ", "r");
    l1->AddEntry("fb2", "Vce ~  8,000 V ", "r");
    l1->SetHeader("Legenda");
    l1->Draw();
