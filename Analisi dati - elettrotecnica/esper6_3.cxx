...
//Percorsi Filippo:
//const char rootfilepath[256]        = "/home/filippo/uni/esp2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - elettrotecnica/esper6.root";
//const char outfilepath[256]         = "/home/filippo/uni/esp2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - elettrotecnica/esper6_results.txt";
//const char transistorTable[256]     = "/home/filippo/uni/esp2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 5 e 6/transistorTable.csv";
...
void esper6_3()
{
    ...
    */
    ...
    for (int j = 0; j < 5; ++j)
    {
        ...
        if ((j != 4) && (j != 2) && (j != 0) && (j != 1) && (j != 3)) a[j] = new TF1(name, "-[0]*exp([1]) * (1+x/[2])", 0.83, 7.28); 
        if (j == 2)               a[j] = new TF1(name, "-[0]*exp([1]) * (1+x/[2])", 2.0, 6.8);
        if (j == 4)               a[j] = new TF1(name, "-[0]*exp([1]) * (1+x/[2])", 2.4, 6.8);
        if (j == 0)               a[j] = new TF1(name, "-[0]*exp([1]) * (1+x/[2])", 1.26, 6.03);
        if (j == 1)               a[j] = new TF1(name, "-[0]*exp([1]) * (1+x/[2])", 2.58, 8.71);
        if (j == 3)               a[j] = new TF1(name, "-[0]*exp([1]) * (1+x/[2])", 2.12, 9.00);
        ...
        ad[j]->SetLineStyle(6);
        printout << "\nChi2 = " << a[j]->GetChisquare() <<  " ndof = " << a[j]->GetNDF() << "  pVal = " << a[j]->GetProb();
        ...
        varSum = varSum + a[j]->GetParError(2) * a[j]->GetParError(2);
        valSum = valSum + a[j]->GetParameter(2);
        valueVa[j] = a[j]->GetParameter(2);
    ...
    //ANOVA
    double var_e  = varSum / 5.;
    double var_mc = 0;
    double val_m  = valSum / 5.;
    int ng        = 5;

    for (int i = 0; i < 5; ++i)
    {
        var_mc = var_mc + (valueVa[i] - val_m) * (valueVa[i] - val_m) / ((double)ng - 1.); //Num gruppi - 1
    }

    double NN        = 5.;
    double var_tra   = NN * var_mc;
    double FF        = var_tra / var_e;
    double pValAnova = st.pvalFright(FF, ng, ng*(NN - 1));

    printout << "\n\nAnova tensioni di Early: F = " << FF << "  pVal = " << pValAnova;
    ...
    int seriesl =  4;
    int series[] = {0, 1, 12, 15};

    TGraphErrors * fb[seriesl];
    TCanvas * cIn = new TCanvas("cIn","Caratteristica di ingresso",10,3,600,400);
    cIn->cd();
    cIn->SetTitle("Caratteristica in ingresso");
    auto * l1 = new TLegend(0.1,0.7,0.48,0.9);
    auto * input1 = new TGraphErrors(5);
    l1->SetHeader("Legenda");
    input1->SetName("InputCFit");

    for (int q = 0; q < seriesl; ++q)
    {
        int j = series[q];
        fb[q] = new TGraphErrors(5);
        fb[q]->SetName((std::string("fb") + std::to_string(j)).c_str());
        fb[q]->SetTitle((std::string("Vce ~ ") + std::to_string(Vce[j+s*2]) + std::string(" V")).c_str());
        for (int i = 0; i < 5; ++i)
        {
            fb[q]->SetPoint(i, Ib[j+i*s], Vbe[j+i*s]);
            fb[q]->SetPointError(i, Ib_err[j+i*s]), Vbe_err[j+i*s];
            if (j == 15)
            {
                input1->SetPoint(i, Ib[j+i*s], Vbe[j+i*s]);
                input1->SetPointError(i, Ib_err[j+i*s], Vbe_err[j+i*s]);
            }
        }
        //fb[j]->SetMarkerStyle(20 + j);
        fb[q]->Sort();
        fb[q]->Write((std::string("fb") + std::to_string(q)).c_str());
        l1->AddEntry(fb[q], fb[q]->GetTitle(), "p");
    }
        
    /*
    TMultiGraph  *mg  = new TMultiGraph();
    
    for (int j = 0; j < seriesl; ++j)
    {
        fb[j]->GetXaxis()->SetLimits(0.4 , 1.8);
        fb[j]->GetYaxis()->SetLimits(3e-5, 9e-4);

        fb[j]->GetXaxis()->SetTitle("Vbe [V]");
        fb[j]->GetYaxis()->SetTitle("Ib [A]");
        fb[j]->SetTitle("Caratteristica in ingresso Ib = f(Vbe)");
        fb[j]->SetMarkerStyle(21 + j);
        fb[j]->SetLineWidth(1);
        fb[j]->SetMarkerSize(1.5);
        mg->Add(fb[j]);
    }

    mg->GetXaxis()->SetTitle("Vbe [V]");
    mg->GetYaxis()->SetTitle("Ib [A]");
    ...
    */
    ...
    //---------------INTERPOLAZIONE CON CURVA DI SHOCKLEY CARATTERISTICA IN INGRESSO--------------
    auto * shockley = new TF1("shockley", "[1]*log(x/[0] + 1) + [2]*x", 0.4637, 10.0);
    shockley->SetParameter(0, 1e-9);
    shockley->SetParameter(1, 5e-2);
    //shockley->SetParLimits(0, 1e-13, 1e-10);
    //shockley->SetParLimits(1, 5e-3, 5e1);
    input1->Fit(shockley);
    printout << "\n\nFit caratteristica in ingresso";
    printout << "\nChi^2  " << shockley->GetChisquare();
    printout << "\np-val  " << shockley->GetProb();
    printout << "\nndof   " << shockley->GetNDF();
    printout << "\nX^2 ridotto   " << shockley->GetChisquare() / shockley->GetNDF();
    printout << "\nIs     " << shockley->GetParameter(0) << " +/- " << shockley->GetParError(0) << " A";
    printout << "\nη*Vt   " << shockley->GetParameter(1) << " +/- " << shockley->GetParError(1) << " V";
    printout << "\nR      " << shockley->GetParameter(2) << " +/- " << shockley->GetParError(2) << " Ω";
    printout << "\n";
    shockley->Write("Input_fit");

    TCanvas * Shockley_c = new TCanvas("cIn","Caratteristica di ingresso",10,3,600,400);
    Shockley_c->cd();
    shockley->SetTitle("Caratteristica in ingresso invertita e interpolata");
    input1->GetXaxis()->SetTitle("Ib [A]");
    input1->GetYaxis()->SetTitle("Vbe [V]");
    input1->SetTitle("Caratteristica in ingresso");
    Shockley_c->SetGrid();
    input1->Draw("AP");
    shockley->Draw("same"); 
    Shockley_c->Write("Shocley_canvas");
    ...
    double eta             = shockley->GetParameter(0)/Vter;      //adim
    double eta_err = sqrt(
        (1/Vter)*(1/Vter)*shockley->GetParError(0)*shockley->GetParError(0)+
        (shockley->GetParameter(0)/(Vter*Vter))*(shockley->GetParameter(0)/(Vter*Vter))*Vter_err*Vter_err
    );
    printout << "\n\nη fattore di idealità   " << eta << " +/- " << eta_err << " adim";
    printout << "\n";*/
