...
/*
    for (int q = 0; q < seriesl; ++q)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            fb[q]->SetPoint(i, Vbe[j+i*s], Ib[j+i*s]);
            fb[q]->SetPointError(i, Vbe_err[j+i*s], Ib_err[j+i*s]);
            if (j == 15)
            {
                input1->SetPoint(i, Vbe[j+i*s], Ib[j+i*s]);
                input1->SetPointError(i, Vbe_err[j+i*s], Ib_err[j+i*s]);
            }
        }
...
*/
    ...
    auto * shockley = new TF1("shockley", "-[0]*(exp(x/[1])-1)", 0.4637, 10.0);
    ...
    printout << "\nR      " << shockley->GetParameter(2) << " +/- " << shockley->GetParError(2) << " Ohm";
    printout << "\n";
    shockley->Write("Input_fit");

    TCanvas * Shockley_c = new TCanvas("cIn","Caratteristica di ingresso",10,3,600,400);
    Shockley_c->cd();
    Shockley_c->SetTitle("Caratteristica in ingresso");
    input1->Draw("AP");
    shockley->Draw("same");
