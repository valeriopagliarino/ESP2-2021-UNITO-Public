...
//Percorsi Valerio:
const char rootfilepath[256]    = "/Users/ValerioPagliarino/Desktop/Esperimentazioni II/Workspace/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - elettrotecnica/esper4.root";
const char outfilepath[256]     = "/Users/ValerioPagliarino/Desktop/Esperimentazioni II/Workspace/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - elettrotecnica/esper4_results.txt";
const char rclfilepath[256]     = "/Users/ValerioPagliarino/Desktop/Esperimentazioni II/Workspace/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 3 e 4/RCLfilter.csv";
const char vrefilepath[256]     = "/Users/ValerioPagliarino/Desktop/Esperimentazioni II/Workspace/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 3 e 4/voltage_res.csv";
const char bandwdtpath[256]     = "/Users/ValerioPagliarino/Desktop/Esperimentazioni II/Workspace/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 3 e 4/bandwidth_vtable.csv";

//Percorsi Federica:
//const char rootfilepath[256]  = "/Users/federicasibilla/Documents/UNI-2/Università-ESP2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - elettrotecnica/esper4.root";
//const char outfilepath[256]   = "/Users/federicasibilla/Documents/UNI-2/Università-ESP2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - elettrotecnica/esper4_results.txt";
//const char rclfilepath[256]   = "/Users/federicasibilla/Documents/UNI-2/Università-ESP2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 3 e 4/RCLfilter.csv";
//const char vrefilepath[256]   = "/Users/federicasibilla/Documents/UNI-2/Università-ESP2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 3 e 4/voltage_res.csv";
//const char bandwdtpath[256]   = "/Users/federicasibilla/Documents/UNI-2/Università-ESP2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati elettrotecnica 3 e 4/bandwidth_vtable.csv";
...
void esper4()
{
    ...
    TF1 * pass_fit    = new TF1("pass_fit", "[3]/(sqrt(([3]+[0])^2+(-[1]*[2]*([3]+[0])/x+[1]*x*([3]+[0])/[2])^2))",2,50000);
    ...
    pass_fit       ->SetParameter(0, 20.);      //Ohm
    pass_fit       ->SetParameter(1, 5.);       //Adimensionale
    pass_fit       ->SetParameter(2, 10000.);   //Hz
    pass_fit       ->FixParameter(3, R);        //Ohm
    ...
    printout << "\nRs    = " << pass_fit->GetParameter(0)   << " +/- " <<  pass_fit->GetParError(0);
    printout << "\nQ     = " << pass_fit->GetParameter(1)   << " +/- " <<  pass_fit->GetParError(1);
    printout << "\nf_r   = " << pass_fit->GetParameter(2)   << " +/- " <<  pass_fit->GetParError(2);
    printout << "\nR !L! = " << pass_fit->GetParameter(3)   << " +/- " <<  pass_fit->GetParError(3);
...
//________________________________________________________Matrice di covarianza_________________________________________
    ...
    // Get fit results
    TFitResultPtr fitResult = band_cut_gain->Fit(cut_fit,"S");

    // Get total covariance matrix (will be used for integral error)
    TMatrixDSym cov = fitResult->GetCovarianceMatrix();
    //TMatrixD cov = pass_fit->GetCovarianceMatrix();

    double Q_fit        = cut_fit->GetParameter(1);
    double Q_fit_var    = cov[1][1];
    double f0_fit       = cut_fit->GetParameter(2);
    double f0_fit_var   = cov[2][2];
    double Rs_fit       = cut_fit->GetParameter(0);
    double Rs_fit_var   = cov[0][0];
 
    double cov_Q_f0     = cov[1][2];
    double cov_Q_Rs     = cov[1][0];
    double cov_f0_Rs    = cov[0][2];
 
    //Ricaviamo L e C:
    double L_fit = (R+Rs_fit) * Q_fit / (2*pi*f0_fit);
    double C_fit = 1./((R+Rs_fit)*2*pi*Q_fit*f0_fit);
    
    double L_fit_err = sqrt(
        (Q_fit/(2*pi*f0_fit)) * (Q_fit/(2*pi*f0_fit)) * Rs_fit_var  +
        ((R+Rs_fit)/(2*pi*f0_fit)) * ((R+Rs_fit)/(2*pi*f0_fit)) * Q_fit_var   +
        (((R+Rs_fit)*Q_fit*2*pi)/((2*pi*f0_fit) * (2*pi*f0_fit))) * (((R+Rs_fit)*Q_fit*2*pi)/((2*pi*f0_fit) * (2*pi*f0_fit))) * f0_fit_var  +
        (Q_fit/(2*pi*f0_fit))  * ((R+Rs_fit)/(2*pi*f0_fit)) * cov_Q_Rs  +
        ((R+Rs_fit)/(2*pi*f0_fit)) * (((R+Rs_fit)*Q_fit*2*pi)/((2*pi*f0_fit) * (2*pi*f0_fit))) * cov_Q_f0 +
        (((R+Rs_fit)*Q_fit*2*pi)/((2*pi*f0_fit) * (2*pi*f0_fit))) * (Q_fit/(2*pi*f0_fit)) * cov_f0_Rs  +
        (Q_fit/(2*pi*f0_fit) *Q_fit/(2*pi*f0_fit)) * R_err * R_err
    );

    double C_fit_err = sqrt(
        2*pi*Q_fit*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*2*pi*Q_fit*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*R_err*R_err+
        2*pi*Q_fit*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*2*pi*Q_fit*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*Rs_fit_var+
        2*(R+Rs_fit)*pi*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*2*(R+Rs_fit)*pi*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*Q_fit_var+
        2*(R+Rs_fit)*pi*Q_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*(2*(R+Rs_fit)*pi*Q_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit)))*f0_fit_var+
        2*pi*Q_fit*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit)) * 2*(R+Rs_fit)*pi*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))* cov_Q_Rs+
        2*pi*Q_fit*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit)) * 2*(R+Rs_fit)*pi*Q_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit)) *cov_f0_Rs+
        2*(R+Rs_fit)*pi*Q_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))* 2*(R+Rs_fit)*pi*f0_fit/((2*(R+Rs_fit)*pi*Q_fit*f0_fit)*(2*(R+Rs_fit)*pi*Q_fit*f0_fit))*cov_Q_f0
    );
