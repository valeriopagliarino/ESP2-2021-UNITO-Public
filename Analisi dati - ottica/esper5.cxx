...
//Percorsi Filippo
//const char rootfilepath[256]       = "/home/filippo/uni/esp2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - ottica/esper5.root";
//const char outfilepath[256]        = "/home/filippo/uni/esp2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Analisi dati - ottica/esper5_results.txt";
//const char IVledCsvp[256]          = "/home/filippo/uni/esp2/ESP2-GR1-FISICA-UNITO-20-21/Fisica - Esperimentazioni 2/Misure e dati sperimentali/Presa dati ottica 4-5/I_V_LED.csv";
...
void esper5()
{
    ...
    */
     /*
    printout << "\n\n\n chi^2 fit istogrammi\n";

    for (int i = 0; i < 5; ++i)
    {
        printout << "\nChi^2 = " << a[i]->GetChisquare();
        //printout << "\nVc0   = " << a[i]->GetParameter(2) << " +/- " << a[i]->GetParError(2);
        Vc0[i]      = a[i]->GetParameter(2);
        Vc0_err[i]  = a[i]->GetParError(2);
    }
    */
    ...
    TF1 * schottky4    = new TF1("schottky4", "[0]*(exp([1]*(x-[2]))-1)",-0.78,-0.41); //  [0]*(exp([1]*(x-[2]))-1)
    schottky4->SetParameter(0, 1e-9); 
    schottky4->SetParameter(1, 0); 
    schottky4->SetParameter(2, -0.5); 

    TF1 * schottky5    = new TF1("schottky5", "[0]*(exp([1]*(x-[2]))-1)",-0.78,-0.41); //  [0]*(exp([1]*(x-[2]))-1)
    schottky5->SetParameter(0, 1e-9); 
    schottky5->SetParameter(1, 0); 
    schottky5->SetParameter(2, -0.5); 

    IV_4->Fit("schottky4","R");
    Vc0[3]     =  abs(schottky4->GetParameter(2));
    Vc0_err[3] =  schottky4->GetParError(2);
    IV_5->Fit("schottky5","R");
    Vc0[4]     =  abs(schottky5->GetParameter(2));
    Vc0_err[4] =  schottky5->GetParError(2);*/
    ...
    TF1 * retta   = new TF1("retta", "[0]*x+[1]",2,50000);
    retta->SetParameter(0, 100.); 
    retta->SetParameter(1, 100.); 
    V_lam->Fit(retta);
    V_lam->Draw();
    ...
    printout << "\nndf   = " << retta->GetNDF();           
    printout << "\np-val = " << retta->GetProb();         
    printout << "\nh     = " << retta->GetParameter(0)   << " +/- " <<  retta->GetParError(0);
    printout << "\nq     = " << retta->GetParameter(1)   << " +/- " <<  retta->GetParError(1);
